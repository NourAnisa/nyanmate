...
#[tauri::command]
fn set_agenda_visible(app: AppHandle, visible: bool) -> Result<(), String> {
    if let Some(window) = app.get_webview_window("agenda") {
        if visible {
            window.show().map_err(|e| e.to_string())?;
            window.set_focus().map_err(|e| e.to_string())?;
        } else {
...
pub fn run() {
    tauri::Builder::default()
        .setup(|app| {
            ...
            let menu = Menu::with_items(app, &[&show, &agenda, &reports, &hide, &quit])?;
        ...
        .invoke_handler(tauri::generate_handler![companion_status, set_presenter_visible, set_reports_visible, set_agenda_visible])
